...
- 데이터 형식은 저장 공간뿐 아니라 성능에도 영향을 미치므로 데이터 특성에 맞게 선택하는 것이 중요함

- 정수형: TINYINT, SMALLINT, INT, BIGINT
  - 크기에 따라 저장 가능한 범위가 다르며, UNSIGNED를 사용하면 음수를 제외하고 양수 범위로 확장 가능
- 문자형: CHAR, VARCHAR
  - CHAR는 고정 길이, VARCHAR는 가변 길이 (공간 효율성 차이 존재)
- 실수형: FLOAT, DOUBLE
  - 소수점 정밀도 차이 (일반적으로 FLOAT로 충분)
- 날짜형: DATE, TIME, DATETIME
  - 날짜, 시간, 날짜+시간 저장 가능

- TEXT / LONGTEXT → 대용량 텍스트 저장
- BLOB / LONGBLOB → 이미지, 영상 등 이진 데이터 저장

- 데이터 형식은 필요에 따라 변환할 수 있음
  - 명시적 변환: CAST(), CONVERT()
  - 암시적 변환: 자동 형 변환
...
- 두 개의 테이블을 연결하여 하나의 결과를 만드는 기능
- 관계가 있는 데이터를 함께 조회할 때 사용

- 개념 
  - 일대다 관계 (PK-FK 관계) : 한 테이블(PK)은 하나, 다른 테이블(FK)은 여러 개 가능

- 주요 조인 종류
  - 내부 조인 (INNER JOIN) : 두 테이블에 모두 존재하는 데이터만 조회
  - 외부 조인 (OUTER JOIN) : 한쪽 테이블에만 있어도 결과 포함
    - LEFT JOIN: 왼쪽 기준
    - RIGHT JOIN: 오른쪽 기준
  - 상호 조인 (CROSS JOIN) : 모든 행 조합 (곱 형태)
  - 자체 조인 (SELF JOIN) : 하나의 테이블을 두 번 사용하는 조인

- 동일한 컬럼명이 있을 경우 테이블명.컬럼명으로 명확히 지정해야 함
- 별칭(alias)을 사용하면 코드 가독성이 좋아짐
...
- WHILE 문 (반복문) : 조건이 참인 동안 반복 실행
  ...
  ```
  - 추가 제어
    - ITERATE → 다음 반복으로 이동 (continue)
    - LEAVE → 반복 종료 (break)

- 변수 사용
  - 선언: DECLARE 변수명 자료형
  - 값 대입: SET 변수 = 값
  - SELECT INTO로 값 저장 가능

- 동적 SQL : 실행 시점에 SQL을 구성하는 방식
  ```sql
  PREPARE 변수 FROM 'SQL문';
  EXECUTE 변수;
